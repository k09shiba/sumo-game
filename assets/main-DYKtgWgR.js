import{G as i,q as k,b as T,s as S,t as h,o as j}from"./index-Cj6Noyl4.js";import{T as E,d as L,e as u,w as B,C as I,f as m,g as R,P as z,D as $,h as C,K as w,i as D,I as F,j as O,k as _}from"./disciple-DjEOdKBc.js";import{g as q,r as X,u as G,b as U,a as W,d as K,c as V}from"./facility-GlCLLL54.js";import{r as Y}from"./events-bNYGPvrJ.js";import{g as J}from"./basho-yp6hZq3g.js";import{r as Q,i as Z}from"./charRenderer-BeLMb1_3.js";function g(t,n,e){const s=t.personality;if(!s||s==="earnest")return Math.round(e*(s==="earnest"?1.25:1));if(s==="genius"){const a=Math.random();return a<.18?Math.max(1,Math.round(e*.3)):a>.82?Math.round(e*2.2):e}return s==="spiritual"?n==="spirit"||n==="motivation"?Math.round(e*1.5):e:s==="technical"&&n==="tech"?Math.round(e*1.5):e}function tt(t,n){var v;const e=E.find(d=>d.id===n);if(!e)return{ok:!1,msg:"不明なコマンドです"};if(t.stamina<e.stamCost)return{ok:!1,msg:`スタミナが足りない！（必要:${e.stamCost}）`};if(e.stamCost>0&&(i.trainTurnsLeft??L)<=0)return{ok:!1,msg:"今月の稽古は十分です！休養してください。"};t.stamina=u(t.stamina-e.stamCost,0,t.maxStamina);const s=1+i.facilities.dojo*.1,a=i.activeItems.chanko_plus&&n==="eat"?1.5:1,o=t.talent/100,l=200+(t.talent-50)*4,c=[];switch(n){case"oshi":{const d=g(t,"power",Math.round(m(8,16)*s*o));t.power=u(t.power+d,0,l),t.styleXP.oshi=(t.styleXP.oshi||0)+10,t.styleXP.oshi>80&&t.sumoStyle!=="oshi"&&(t.sumoStyle="oshi",c.push("押し相撲が身についた！")),c.push(`筋力 +${d}`);break}case"yotsu":{const d=g(t,"tech",Math.round(m(8,16)*s*o));t.tech=u(t.tech+d,0,l),t.styleXP.yotsu=(t.styleXP.yotsu||0)+10,t.styleXP.yotsu>80&&t.sumoStyle!=="yotsu"&&(t.sumoStyle="yotsu",c.push("四つ相撲が身についた！")),c.push(`技術 +${d}`);break}case"nage":{const d=g(t,"tech",Math.round(m(5,11)*s*o));t.tech=u(t.tech+d,0,l),t.styleXP.tech=(t.styleXP.tech||0)+8;const r=m(1,2);t.weight=u(t.weight-r,60,250),c.push(`技術 +${d}、体重 -${r}kg`);break}case"run":{const d=Math.round(m(5,14)*s);t.maxStamina=u(t.maxStamina+d,0,200),t.weight=u(t.weight-1,60,250),c.push(`体力上限 +${d}、体重 -1kg`);break}case"mental":{const d=g(t,"spirit",Math.round(m(8,16)*s*o)),r=g(t,"motivation",m(5,12));t.spirit=u(t.spirit+d,0,l),t.motivation=u(t.motivation+r,0,100),c.push(`精神 +${d}、やる気 +${r}`);break}case"diet":{const d=t.optimalWeight,r=t.weight-d;if(Math.abs(r)<=5)c.push("すでに適正体重！");else if(r>0){const p=Math.min(r,m(3,6));t.weight-=p,c.push(`体重 -${p}kg（適正体重へ近づいた）`)}else{const p=Math.min(-r,m(2,4));t.weight+=p,c.push(`体重 +${p}kg（適正体重へ近づいた）`)}break}case"group":{const d=i.facilities.dojo+1,r=g(t,"power",Math.round(m(3,6)*d*o));t.power=u(t.power+r,0,l),t.tech=u(t.tech+r,0,l),t.spirit=u(t.spirit+r,0,l),c.push(`全ステ +${r}`);break}case"cond":{const d=t.conditionIdx;t.conditionIdx=u(t.conditionIdx+1,0,4),t.conditionIdx>d?c.push(`コンディション：${I[d].label} → ${I[t.conditionIdx].label}`):c.push("コンディションはすでに最高！");break}case"sleep":{t.stamina=t.maxStamina;const d=m(10,18);t.motivation=u(t.motivation+d,0,100),c.push(`スタミナ全回復！やる気 +${d}`);break}case"eat":{const d=1+i.facilities.kitchen*.2,r=B(t);let p,f;r==="optimal"?(p=m(2,4),f=Math.round(m(4,10)*d*a)):r==="overweight"?(p=m(1,2),f=Math.round(m(2,5)*d*a)):(p=m(3,6),f=Math.round(m(5,12)*d*a)),t.weight=u(t.weight+p,60,250),t.maxStamina=u(t.maxStamina+f,0,200),t.stamina=u(t.stamina+f,0,t.maxStamina),c.push(`体重 +${p}kg、体力 +${f}`);break}}if(e.stamCost>0&&(i.trainTurnsLeft=Math.max(0,(i.trainTurnsLeft??L)-1)),t.personality==="genius"&&c.length>0){const d=parseInt(((v=c[0].match(/\d+/))==null?void 0:v[0])||"0"),r=12;d>r*1.8?c.unshift("✨ 天才的なひらめき！大爆発！"):d<r*.5&&d>0&&c.unshift("😓 今日はイマイチ…")}return e.stamCost>0&&t.motivation<30&&c.push("やる気が低い…稽古の効果が薄い。"),t.injuryLevel>=1&&e.stamCost>=20&&(c.push("怪我が痛む…"),t.stamina=u(t.stamina-5,0,t.maxStamina)),nt(t),{ok:!0,msgs:c}}function nt(t){if(t.injuryLevel===0)return;const e=.15+i.facilities.medical*.12;return i.activeItems.kampo,Math.random()<e?(t.injuryLevel=Math.max(0,t.injuryLevel-1),`${t.name}の怪我が回復した！`):null}function b(){const t=document.getElementById("hd-info");if(!t)return;const n=i.month,e=i.year;t.textContent=`令和${e}年${n}月  💰${i.ryo}両`}function y(){const t=i.disciples[i.focusIdx];if(!t)return;Q(t);const n=document.getElementById("tr-name");n&&(n.textContent=t.name);const e=document.getElementById("tr-rank");e&&(e.textContent=R(t));const s=document.getElementById("tr-cond");if(s){const l=I[t.conditionIdx??2];s.textContent=l.label,s.style.color=l.color}const a=document.getElementById("tr-personality");if(a){const l=z.find(c=>c.id===t.personality);l&&(a.textContent=`${l.icon} ${l.name}`)}const o=document.getElementById("train-turns-info");if(o){const l=i.trainTurnsLeft??10,c=l<=2?"#e74c3c":l<=4?"#e67e22":"#27ae60";o.innerHTML=`<span class="turns-label">稽古残り</span> <span class="turns-count" style="color:${c}">${l}</span><span class="turns-label"> / 10回</span>`}st(t),et(t)}function st(t){const n=document.getElementById("stat-bars");if(!n)return;const e=999,s=[{label:"筋力",val:t.power,max:e,col:"#c0392b"},{label:"技術",val:t.tech,max:e,col:"#2980b9"},{label:"精神",val:t.spirit,max:e,col:"#8e44ad"},{label:"体力",val:t.stamina,max:t.maxStamina,col:"#27ae60"},{label:"やる気",val:t.motivation,max:100,col:"#f39c12"},{label:"体重",val:`${t.weight}kg`,raw:!0}];if(n.innerHTML=s.map(o=>{if(o.raw){const c=B(t),v=c==="optimal"?"#27ae60":c==="overweight"?"#e74c3c":"#999";return`
        <div class="stat-row">
      ...
      <div class="disciple-card retired">
        <span>${n.name}（引退）</span>
        <small>最高位:${$[n.divIdx].short}　${n.totalWins}勝${n.totalLosses}敗</small>
      </div>`;const s=$[n.divIdx],a=I[n.conditionIdx??2];return`
    ...
    </div>`).join("")}function M(t,n=""){const e=document.getElementById("msg-log");if(!e)return;const s=document.createElement("div");for(s.className=`msg ${n}`,s.textContent=t,e.prepend(s);e.children.length>30;)e.removeChild(e.lastChild)}function ct(t){var a;const n=i.disciples[t];if(!n||n.divIdx<4)return;const e=n.keshoMawashi||w[0].id,s=((a=w.find(o=>o.id===e))==null?void 0:a.name)||"赤獅子";k({em:"🎽",title:"化粧まわし変更",text:`現在：${s}
化粧まわしを選んでください。`,choices:w.map(o=>({label:`🎽 ${o.name}（${o.desc}）${o.id===e?" ◀現在":""}`,fn:()=>{n.keshoMawashi=o.id,T(),y()}}))})}function rt(t){if(!t)return;const n=t;n.innerHTML=`<h3 class="section-title">番付表 ─ 令和${i.year}年${i.month}月</h3>`;const e=i.disciples.filter(a=>!a.retired),s=i.rival;if(i.sponsors&&i.sponsors.length>0){const a=document.createElement("div");a.className="sponsor-list",a.innerHTML=`<h4 class="sub-title">現スポンサー（${i.sponsors.length}社）</h4>`,a.innerHTML+=i.sponsors.map(o=>`<div class="sponsor-row"><span>${o.icon} ${o.name}</span><span class="sponsor-income">+${o.income}両/場所</span></div>`).join(""),n.appendChild(a)}for(let a=$.length-1;a>=0;a--){const o=$[a],l=e.filter(r=>r.divIdx===a),c=s&&s.divIdx===a;if(l.length===0&&!c){const r=document.createElement("div");r.className="banzuke-section banzuke-empty-section",r.innerHTML=`<div class="banzuke-div-name">${o.name}</div><div class="banzuke-no-member">（所属力士なし）</div>`,n.appendChild(r);continue}const v=document.createElement("div");v.className="banzuke-section",v.innerHTML=`<div class="banzuke-div-name">${o.name}</div>`,[...l.map(r=>({name:r.name,pos:r.pos,isPlayer:!0,d:r})),...c?[{name:s.name,pos:s.pos,isRival:!0}]:[]].sort((r,p)=>r.pos-p.pos).forEach(r=>{const p=r.isPlayer&&i.focusIdx===i.disciples.indexOf(r.d),f=r.isPlayer?p?"⭐":"★":"⚔",N=r.isPlayer?"banzuke-player"+(p?" banzuke-focus":""):"banzuke-rival",P=r.isPlayer?`${r.d.wins}勝${r.d.losses}敗`:"";v.innerHTML+=`
        ...
        </div>`}),n.appendChild(v)}}function lt(){var e,s;const t=document.getElementById("screen-main");t.innerHTML=`
    <div class="main-wrap">
      <!-- ヘッダー -->
...
${((o=t.yushoRecord)==null?void 0:o.length)>0?`${t.yushoRecord.length}回の優勝を誇る名力士だった。`:"誠実な相撲で多くのファンを魅了した。"}`),k({em:e,title:s,text:a,onOk:()=>{X(t),i.focusIdx=i.disciples.findIndex(l=>!l.retired)||0,lt()}})}window.doUpgradeFacility=function(t){const n=G(t);if(!n.ok){h(n.msg);return}const e=C[t];h(`${e.name}を強化しました！`),b(),x(document.getElementById("facility-content"))};window.doBuyItem=function(t){const n=U(t);if(!n.ok){h(n.msg);return}h("アイテムを購入しました！"),b(),x(document.getElementById("facility-content"))};window.doBuySpecialTraining=function(t){const n=i.disciples[i.focusIdx],e=W(n,t);if(!e.ok){h(e.msg);return}e.msgs.forEach(s=>M(s,"mge")),h("特別稽古を実施しました！"),b(),y()};window.doPrestigeAction=function(t){const n=K(t);if(!n.ok){h(n.msg);return}h("施策を実施しました！"),b(),x(document.getElementById("facility-content"))};window.doScoutAction=function(t){const n=V(t);if(!n.ok){h(n.msg);return}h("スカウト施策を実施しました！"),b()};window.doSave=function(){T(),h("セーブしました！")};function mt(){if(i.disciples.filter(n=>!n.retired).length===0){h("まず弟子を迎えてください！");return}i.phase="basho",S("basho")}function pt(){window.openSettings=j,window.handleTraining=window.handleTraining,window.setFocusDisciple=window.setFocusDisciple,window.setFocusAndTab=window.setFocusAndTab,window.confirmRetire=window.confirmRetire,window.doUpgradeFacility=window.doUpgradeFacility,window.doBuyItem=window.doBuyItem,window.doBuySpecialTraining=window.doBuySpecialTraining,window.doPrestigeAction=window.doPrestigeAction,window.doScoutAction=window.doScoutAction,window.doSave=window.doSave}export{lt as renderMain};
